...
| Room-01 metric/collision World R3 | partial | Accepted 168-camera registration, metric seed, registered evidence-mesh review, and fail-closed collision preview gates exist | Recover or reproduce the self-contained package, replace the truncated mesh, validate occupancy/free space and doorway clearance, then run the reference route |
