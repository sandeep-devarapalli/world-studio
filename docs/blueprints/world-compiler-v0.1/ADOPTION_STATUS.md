...
| Capability-routed physics runtime | partial | Layered OpenUSD synthetic smoke compiler, additive backend requirements, Newton 1.5.0 Apple CPU synthetic and Room-01 proposal receipts, supervised SuperDex 1.0.0 lifecycle, a deterministic checksum-bound canonical World-to-SuperDex static scene compiler, and supervised compiled-scene load/contact/three-reset receipts; all Room-01 authority remains false | Solver-neutral state client, Room-01 PLY compilation and execution, promoted-collider and local/remote parity, cutover, and Rapier removal |
| SuperDex contact-rich profile | partial | Pinned Physics/Robotics packages, package/license boundary, routing policy, supervised bounded worker lifecycle, synthetic rigid contact evidence, and a native Apple ARM CPU receipt for checksum-bound compiled-World scene load, named-surface contact, and three full-state resets; authority is compiled-scene execution only | Room-01 PLY ingestion, robot Asset compiler, tactile/deformable probes, Franka tabletop A/B, and measured external reference |
