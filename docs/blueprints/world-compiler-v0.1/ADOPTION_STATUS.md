...
| Capability-routed physics runtime | partial | Layered OpenUSD synthetic smoke compiler, additive backend requirements, Newton 1.5.0 Apple CPU synthetic and Room-01 proposal receipts, plus a strict one-shot SuperDex 1.0.0 capability/contact/reset probe; all Room-01 authority remains false | Solver-neutral client, supervised Newton/SuperDex product workers, derived SuperDex scene compiler, promoted-collider and local/remote parity, cutover, and Rapier removal |
| SuperDex contact-rich profile | partial | Pinned Physics/Robotics packages, package/license boundary, routing policy, strict capability report, and three-repeat synthetic rigid contact/point-force/reset execution on Apple ARM CPU; authority is software-only | Product worker supervision, derived scene/robot assets, tactile/deformable probes, Franka tabletop A/B, and measured external reference |
