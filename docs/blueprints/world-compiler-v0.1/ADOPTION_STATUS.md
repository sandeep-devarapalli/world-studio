...
| Capability-routed physics runtime | partial | Layered OpenUSD synthetic smoke compiler, additive backend requirements, plus Newton 1.5.0 Apple CPU synthetic and Room-01 proposal receipts; all Room-01 authority remains false and SuperDex execution is not integrated | Solver-neutral client, separate Newton/SuperDex workers, derived SuperDex scene compiler, promoted-collider and local/remote parity, cutover, and Rapier removal |
| SuperDex contact-rich profile | planned | Upstream revision, package/license boundary, routing policy, and M5A task are recorded | Pinned worker capability report, derived scene/robot assets, contact/reset smoke, Franka tabletop A/B, and measured external reference |
